...
g_mg8 <- dat_temp %>% 
  ...
  theme_mg2_mff() + theme(legend.position = "none")
g_mg8
...
dat_long %>% 
  ...
  labs(title = "Matchutfall. Malmö FF i allsvenskan för herrar", 
...
# MFF games in Allsvenskan 2008-2020, mg10 ----
dat_long %>% 
  filter(Fokuslag == "Malmö FF", Säsong %in% 2008:2020) %>% 
  mutate(Säsong = ordered(Säsong, 2020:2008)) %>% 
...
dat_long %>% 
  ...
  scale_y_continuous(breaks = -2008:-2020, labels = 2008:2020) +
  scale_x_continuous(breaks = seq(0,10,5)) +
  scale_color_manual(values = c(mff_cols[2], "white", mff_cols[4])) +
  facet_wrap(~ Plats) +
  labs(title = "Matchutfall. MFF i herrallsvenskan, 2008 - 2020",
       x = "Omgång", y = "Säsong") +
  theme_mg2_mff() +
  theme(panel.grid.major.x = element_blank(),
...
ggplot(dat_temp, aes(Matchnr, Kumulativ_summa, color = Plats)) +
  geom_step(size = 1.5) +
  ...
  labs(title = "Vunna minus förlorade, mot antal spelade matcher. MFF i herrallsvenskan, 1931 - 2020",
...
dat_temp %>% 
  ...
  scale_x_continuous(breaks = seq(1900, 2100, 10)) +
  scale_color_manual(values = c(mff_cols[2], mff_cols[4])) +
  labs(title = "Gjorda och insläppa mål. Hemma och borta. MFF i herrallsvenskan, 1931 - 2020",
...
ggplot(dat_temp, aes(`Gjorda mål, medel`, `Insläppta mål, medel`)) + 
...
ggplot(dat_temp, aes(ifelse(Plats == "Hemma", -Matchnr, Matchnr), 
  ...
  labs(title = "Aggregerat resultat, vinster -förluster. MFF i herrallsvenskan 1931 - 2020", 
       x = "Antal matcher", y = "Aggregerat resultat, vinster - förluster") +
  theme_mg2_mff() +
  theme(legend.position = "none",
